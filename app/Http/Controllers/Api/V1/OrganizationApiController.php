...
class OrganizationApiController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $query->withCount(['users'])->addSelect([
                'organizations.*', // Pastikan semua field organization tetap ada

                // Count datasets yang approved dan publik
                'total_datasets' => DB::table('datasets')->join('users', 'datasets.user_id', '=', 'users.id')->whereColumn('users.organization_id', 'organizations.id')->where('datasets.approval_status', 'approved')->where('datasets.classification', 'publik')->selectRaw('count(*)'),

                // Count infografis yang public
                'total_infografis' => DB::table('infografis')->join('users', 'infografis.user_id', '=', 'users.id')->whereColumn('users.organization_id', 'organizations.id')->where('infografis.is_public', true)->selectRaw('count(*)'),

                // Count mapsets
                'total_mapsets' => DB::table('mapsets')->join('users', 'mapsets.user_id', '=', 'users.id')->whereColumn('users.organization_id', 'organizations.id')->selectRaw('count(*)'),

                // Count visualisasi yang active dan public
                'total_visualisasi' => DB::table('visualisasi')->join('users', 'visualisasi.user_id', '=', 'users.id')->whereColumn('users.organization_id', 'organizations.id')->where('visualisasi.is_active', true)->where('visualisasi.is_public', true)->selectRaw('count(*)'),
            ]);

            // Pagination
            $perPage = max(1, min((int) $request->get('per_page', 100), 100));
            ...
            $organizations->getCollection()->transform(function ($organization) {
                ...
                // Total content
                $organization->total_content = $organization->total_datasets + $organization->total_infografis + $organization->total_mapsets + $organization->total_visualisasi;
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $organizations = $query->orderBy('name', 'asc')->paginate($perPage);

            // Process results
            $organizations->getCollection()->transform(function ($organization) {
                $organization->total_content = $organization->total_datasets + $organization->total_infografis + $organization->total_mapsets + $organization->total_visualisasi;

                $organization->logo_url = $organization->logo ? asset('storage/' . $organization->logo) : null;
    ...
    public function getTopContributors(Request $request): JsonResponse
    {
        ...
        try {
            $organizations = Organization::withCount([
                ...
                ->get()
                ->map(function ($org) {
                    $org->total_contributions = $org->total_datasets + $org->total_infografis + $org->total_mapsets + $org->total_visualisasi;
            ...
            $organizations = Organization::withCount([
                ...
                ->map(function ($organization) {
                    return [
                        ...
                        'total_content' => $organization->total_datasets + $organization->total_infografis + $organization->total_mapsets + $organization->total_visualisasi,
