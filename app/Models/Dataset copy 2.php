...
class Dataset extends Model
{
    ...
    // 🎯 UPDATED: Approval status constants with revision
    const APPROVAL_PENDING = 'pending';
    const APPROVAL_APPROVED = 'approved';
    const APPROVAL_REJECTED = 'rejected';
    const APPROVAL_REVISION = 'revision'; // 🆕 NEW STATUS
    ...
    // 🎯 UPDATED: Approval methods with revision support
    ...
    // 🆕 NEW: Revision status check
    public function isRevision(): bool
    {
        return $this->approval_status === self::APPROVAL_REVISION;
    }

    // 🆕 NEW: Check if dataset needs attention (revision or rejected)
    public function needsAttention(): bool
    {
        return in_array($this->approval_status, [self::APPROVAL_REVISION, self::APPROVAL_REJECTED]);
    }
    ...
    // 🆕 NEW: Check if dataset can be resubmitted
    public function canBeResubmitted(): bool
    {
        return in_array($this->approval_status, [self::APPROVAL_REJECTED, self::APPROVAL_REVISION]);
    }
    ...
    // 🎯 UPDATED: Approval scopes with revision
    ...
    {
        return $query->where('approval_status', self::APPROVAL_REVISION);
    }

    // 🆕 NEW: Scope for datasets needing attention
    public function scopeNeedsAttention($query)
    {
        return $query->whereIn('approval_status', [self::APPROVAL_REVISION, self::APPROVAL_REJECTED]);
    }
    ...
    // 🆕 NEW: Request revision method
    public function requestRevision($reviewerId, $reason, $notes = null)
    {
        return $this->update([
            'approval_status' => self::APPROVAL_REVISION,
    ...
    // 🎯 UPDATED: Get approval statuses with revision
    public static function getApprovalStatuses()
    {
        return [
            ...
            self::APPROVAL_REVISION => 'Needs Revision' // 🆕 NEW STATUS
        ];
    }
